...
const props = defineProps<{
  ...
  targetBasedFees?: { description: string; amountPerUnit: number; unit: string; estimatedMonthlyUnits?: number }[]
...
// --- 8-Point Grid & Harmonized Quotation Styles (matches shoot-contract) ---
const styles = {
  ...
  <Document title="Retainer Agreement" :author="organizationName" creator="Modest Human Brands" producer="MDoc">
    <Page size="A4" :style="[styles.page, { fontFamily: organizationFont }]">
      ...
      <View :style="styles.headerRow">
        ...
        <View :style="styles.metaSection">
          <View :style="styles.titleContainer">
            <Text :style="{ fontSize: 24, color: props.organizationColorAccent, fontWeight: 'bold' as const, textAlign: 'right' }"> Retainer Agreement </Text>
            <Text :style="{ fontSize: 12, marginTop: 16 }">{{ engagementTitle }}</Text>
          </View>
      ...
      <View :style="{ ...styles.infoBanner, backgroundColor: organizationColorAccent + '33' }">
        <View :style="styles.bannerCol">
          ...
          <Text :style="{ fontSize: 12 }">{{ engagementTitle }}</Text>
      ...
      <Text :style="{ ...styles.sectionTitle, marginTop: 24, marginBottom: 24 }">MONTHLY SCOPE OF WORK</Text>
      ...
      <View v-for="(item, index) in scopeOfWork" :key="index" :style="styles.tableRow" :wrap="false">
      ...
      <View :style="{ marginBottom: 32 }">
        <View v-if="flatMonthlyFee" :style="styles.financialRow" :wrap="false">
          <Text :style="{ ...styles.colLeftSpan, fontWeight: 'bold', fontSize: 12 }">Flat Monthly Fee</Text>
          <Text :style="styles.colAmount">{{ formatCurrency(flatMonthlyFee) }} / month</Text>
        </View>

        <View v-for="(target, tIndex) in targetBasedFees" :key="tIndex" :style="styles.financialRow" :wrap="false">
          <Text :style="{ ...styles.colLeftSpan, color: '#888888', fontSize: 12 }"> Target-Based Fee &middot; {{ target.description }} </Text>
          <Text :style="{ ...styles.colAmount, color: '#888888' }">{{ formatCurrency(target.amountPerUnit) }} / {{ target.unit }}</Text>
        </View>

        <View v-if="onboardingFee" :style="styles.financialRow" :wrap="false">
          <Text :style="{ ...styles.colLeftSpan, color: '#888888', fontSize: 12 }">Onboarding Fee (one-time)</Text>
          <Text :style="{ ...styles.colAmount, color: '#888888' }">{{ formatCurrency(onboardingFee) }}</Text>
        </View>
      </View>
    ...
    <Page size="A4" :style="[styles.page, { fontFamily: organizationFont }]">
      ...
      <Text :style="styles.acceptanceSub">
        ...
        >, accept the engagement structure and choose to proceed under the terms and conditions detailed above.
      </Text>
